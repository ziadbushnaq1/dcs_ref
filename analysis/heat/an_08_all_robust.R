# an_08_all_robust.R —  robustness: FE ladder, clustering,
...
# (existing DuckDB load block from an_08_landsat_all.R, unchanged, -> pixel_data)
...
params <- bind_rows(
  ...
  anchor %>% mutate(contam_scope = "hs_only"),        # <- your requested run
  anchor %>% mutate(contam_timing = "static_dated"),  # = old drop_unknown FALSE
